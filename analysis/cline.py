...
def scan(path: pathlib.Path) -> Scan:
    ...
    meta = {
        ...
        "task": dg.clip(str(index_item["task"]), 120)
...
def _tool_event_from_ui(ts: float, payload: dict, model, counters: Counter) -> dg.Ev | None:
    ...
    if name == WRITE_FILE_TOOL and isinstance(payload.get("content"), str):
        ...
        ev.text = dg.clip(str(payload.get("regex", "")), 80)
    elif name in ("web_fetch", "web_search"):
        ev.text = dg.clip(str(payload.get("path") or payload.get("content") or ""), 100)
    return ev
        ...
        for m in s.ui:
            ...
            if t == "say" and kind in ("task", "user_feedback"):
                ...
                if human is None:
                    ...
                    counters["prompt"] += 1
                    _emit(dg.Ev(0, ts, "prompt", dg.clip(human, dg.PROMPT_MAX)))
                    ...
                    if isinstance(info.get("streamingFailedMessage"), str) and _in_window(ts):
                        _emit(
                            dg.Ev(
                                ...
                                ts,
                                "result_error",
                                dg.clip(info["streamingFailedMessage"], dg.ERROR_MAX),
                                tool="api_request",
                                ok=False,
                            )
                        )
            ...
            for b in parts:
                ...
                if role == "user" and bt == "text":
                    ...
                    if human is None:
                        ...
                        counters["prompt"] += 1
                        _emit(dg.Ev(0, base, "prompt", dg.clip(human, dg.PROMPT_MAX)))
                    ...
                    if _in_window(base):
                        ...
                        _emit(
                            dg.Ev(
                                ...
                                base,
                                "assistant",
                                dg.clip(b["text"], dg.ASSISTANT_MAX),
                                model=model,
                            )
                        )
    ...
    for tid, b in api_results:
        ...
        _emit(
            dg.Ev(
                ...
                ev.ts,
                "result_error",
                dg.clip(msg or "(error)", dg.ERROR_MAX),
                tool=ev.tool,
                path=ev.path,
                ok=False,
                tool_id=tid,
