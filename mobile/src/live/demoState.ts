...
/** `demo_push.PHASE_OF_STEP`: the card's phase for the in-app island's step; `clear` has no card. */
...
/** `demo_push.RANK`: a card only ever moves forward. */
export const DEMO_RANK: Readonly<Record<DemoPhase, number>> = { asked: 0, filming: 1, ready: 2, failed: 2 };
 ...
 * phase but failed.
 */
export function demoState(req: DemoRequestLike | null | undefined, opts: { nowMs: number }): DemoState | null {
  const step = demoStepFor(req?.status);
  if (step === 'clear' || !req) return null;
  const phase = DEMO_PHASE_OF_STEP[step];
...
/** The stale date a phase is sent with, in seconds from now. */
...
/**
 * The demo cards to take down now, when the app comes to the front: answered at least `holdMs`
 * ago, or past their stale date. A kit that landed while Builda was closed has been seen by the
...
export function demosToEnd(
  ...
  return cards
    .filter((c) => c.state === 'stale' || (c.state === 'active' && isAnswered(c.phase) && nowMs - c.updatedEpoch * 1000 >= holdMs))
    .map((c) => c.requestId);
}
