...
the contract's own objects (`burn`, `title_ids`: numbers, bools and enums, emitted into
contract.py beside SessionUpload), the `live` and `live_names` fields (shaped by
...
# `burn`, `title_ids`) and the `quotes` document are written by the same code as the
# analysis, report and live specs, and `int, nullable` means one thing everywhere.
...
#: server's hook channel, `burn` and `title_ids` from the analysis modules capture runs.
#: No Swift type encodes them, so on the Mac they stay unrepresentable in practice; the
        ...
        json.dumps(
            {
                ...
                "note": (
                    ...
                    "live_names, burn, title_ids) so a scalar-path walk of a real payload "
