...
they follow the text colour. The -color ones keep their brand colours. Aider has no mark of its
own and keeps its pixel glyph (src/pixel/harness.ts, the `>_` prompt) on the phone; the native
surfaces draw THAT glyph too, read from the same frame and filled with the tint like the other
monochrome marks (`PIXEL_MARKS`). FOUND IN THE CAPTURE PASS (2026-09-14): the Lock Screen card for
an Aider session had no mark at all beside a row of cards that each had one.
...
# wire harness id -> the pixel glyph a native surface draws for a tool with no SVG: the frame's
# name in src/pixel/harness.ts, the one place it is drawn. Swift only: the phone draws the glyph
# itself (`HarnessLogo` falls back to `HarnessGlyph`), so harnessLogos.ts does not list these.
PIXEL_MARKS = {"aider": ("aider-pixel", "AIDER")}
PIXEL_SRC = ROOT / "mobile/src/pixel/harness.ts"
...
def marks() -> dict[str, dict]:
    ...
    for stem, const in PIXEL_MARKS.values():
        out[stem] = {"layers": [{"segments": pixel_segments(pixel_frame(const)), "fill": {"kind": "tint"}, "evenodd": False}], "mono": True}
    return out


def pixel_frame(const: str) -> list[str]:
    """`const NAME: Frame = [ '....', ... ];` from src/pixel/harness.ts: 16 rows of 16 cells, `b`
    drawn and `.` empty, the frame format `frames.ts` reads."""
    src = PIXEL_SRC.read_text()
    m = re.search(rf"const {const}: Frame = \[(.*?)\];", src, re.S)
    if not m:
        raise SystemExit(f"gen_harness_logos.py: no frame {const} in {PIXEL_SRC.relative_to(ROOT)}")
    rows = re.findall(r"'([.b]+)'", m.group(1))
    if len(rows) != 16 or any(len(r) != 16 for r in rows):
        raise SystemExit(f"gen_harness_logos.py: frame {const} is not 16 rows of 16 cells")
    return rows


def pixel_segments(rows: list[str]) -> list[tuple]:
    """The frame's drawn cells as rectangles on the 24 unit grid (1.5 units a cell), one per run
    of cells in a row, so the path is as simple as the drawing."""
    cell = VIEWBOX / len(rows)
    out: list[tuple] = []
    for y, row in enumerate(rows):
        for m in re.finditer(r"b+", row):
            x0, x1 = m.start() * cell, m.end() * cell
            y0, y1 = y * cell, (y + 1) * cell
            out += [("M", x0, y0), ("L", x1, y0), ("L", x1, y1), ("L", x0, y1), ("Z",)]
    return out
...
def gen_swift(ms: dict[str, dict]) -> str:
    ...
    wire = {**MAP, **{w: stem for w, (stem, _) in PIXEL_MARKS.items()}}
    wire_cases = "\n".join(f'    case "{w}": return "{s}"' for w, s in wire.items())
    mono = ", ".join(f'"{s}"' for s in stems if ms[s]["mono"])
    return f"""// GENERATED by scripts/gen_harness_logos.py from mobile/assets/harness/*.svg and Aider's pixel
